function [om_new T_new KK_left_new KK_right_new,filenamelist_L,filenamelist_R]=rectify_stereo(KK_left,kc_left,KK_right,kc_right,om,T,filenamelist_L,filenamelist_R,image_size,PathNameBase,pname2)

% 输入参数还要加上 图像像素大小 nx ny
nx = image_size(1,2);    % 图像宽度
ny = image_size(1,1);    % 图像高度
cc_left(1,1) = KK_left(1,3);
cc_left(2,1) = KK_left(2,3);
fc_left(1,1) = KK_left(1,1);
fc_left(2,1) = KK_left(2,2);
alpha_c_left = KK_left(1,2)/KK_left(1,1);
cc_right(1,1) = KK_right(1,3);
cc_right(2,1) = KK_right(2,3);
fc_right(1,1) = KK_right(1,1);
fc_right(2,1) = KK_right(2,2);
alpha_c_right = KK_right(1,2)/KK_right(1,1);
