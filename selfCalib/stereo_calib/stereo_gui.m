...
cell_list{1,1} = {'加载左右相机标定结果数据','load_stereo_calib_files;'};
cell_list{1,2} = {'开始双目标定程序','go_calib_stereo;'};
cell_list{2,1} = {'显示标定板的外参','ext_calib_stereo;'};
cell_list{2,2} = {'显示内参','show_stereo_calib_results;'};
cell_list{3,1} = {'保存双目标定结果','saving_stereo_calib;'};
cell_list{3,2} = {'加载双目标定结果数据','loading_stereo_calib;'};
cell_list{4,1} = {'修正标定图像','rectify_stereo_pair;'};
cell_list{4,2} = {'退出',['disp(''再见，双目相机标定已完成。''); close(' num2str(fig_number) ');']}; %{'Exit','calib_gui;'};


show_window(cell_list,fig_number,title_figure,400,30);
