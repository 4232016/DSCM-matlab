...
if ~exist('nx')&~exist('ny'),
    fprintf(1,'提示:程序默认标定图像大小为： nx=640 and ny=480. 若这个数值不正确，请手动修改。\n');
    nx = 640;
    ny = 480;
end;
...
if center_optim & (length(ind_active) < 2) & ~rig3D,
    fprintf(1,'警告: 当只有一副标定图像时，光轴点无法确认。\n');
...
if center_optim & (length(ind_active) < 5) & ~rig3D,
    fprintf(1,'警告: 标定图像少于5幅，光轴点预判可能不准确.\n');
end;
...
while (change > 1e-9) && (iter < MaxIter),
 ...
 % -------------------------------The first step------------------ ----
 %consists of updating the whole vector of knowns (intrinsic + extrinsic of active images) through a one step steepest gradient descent.
    f = param(1:2);
    c = param(3:4);
    alpha = param(5);
    k = param(6:10);
...
% ------------------------Second step: (optional) ---------------------------- 
    %It makes convergence faster, and the region of convergence LARGER!!!
