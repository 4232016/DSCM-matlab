function [grid_x_L,grid_y_L]=grid_generator(FileNameBase,PathNameBase, grid_x_L, grid_y_L);
...
% text based format where the x-position is saved as grid_x_l.dat and the
% y-position saved as grid_y_l.dat.
%
...
if exist('FileNameBase','var')~=1
    ...
    {'*.bmp;*.tif;*.jpg;*.TIF;*.BMP;*.JPG','图像文件 (*.bmp,*.tif,*.jpg)';'*.*',  '所有文件 (*.*)'}, ...
    '打开画网格的基础图像（左相机图像）');
else 
    FilterIndex=1;
end
...
if FilterIndex~=0 
     ...
     [grid_x_L,grid_y_L,FileNameBase,PathNameBase] = gridtypeselection(FileNameBase, PathNameBase, im_grid, grid_x_L, grid_y_L,1); %call gridselection
...
%% Main Menu
function [grid_x_L,grid_y_L,FileNameBase,PathNameBase] = gridtypeselection(FileNameBase, PathNameBase, im_grid, grid_x_L, grid_y_L,loadgrid);
...
if loadgrid
    ...
    if loadoldgrid==1
        [gridxname,Pathgridx] = uigetfile('*.dat','Open grid_x_l.dat');     %load grid_x_l
            if gridxname==0
                disp('请选择相应的数据文件!')
            end
        cd(Pathgridx);
        grid_x_L=importdata(gridxname,'\t');

        [gridyname,Pathgridy] = uigetfile('*.dat','Open grid_y_l.dat');%load grid_y_l
            if gridyname==0
                disp('请选择相应的数据文件!')
            end
        cd(Pathgridy);
        grid_y_L=importdata(gridyname,'\t');
...
gridselection = menu(sprintf('Gridgenerator Main Menu'),...
    ...
    % Rectangular
    if gridselection==1
        [grid_x_L,grid_y_L,FileNameBase,PathNameBase] = rect_grid(FileNameBase, PathNameBase, im_grid, grid_x_L, grid_y_L);
        
    end
    % Circular
    if gridselection==2
        [grid_x_L,grid_y_L,FileNameBase,PathNameBase] = circ_grid(FileNameBase, PathNameBase, im_grid, grid_x_L, grid_y_L);
        
    end
    % Two Markers
    if gridselection==3
        [grid_x_L,grid_y_L,FileNameBase,PathNameBase] = twop_grid(FileNameBase, PathNameBase, im_grid, grid_x_L, grid_y_L);
        
    end
    % Line
    if gridselection==4
        [grid_x_L,grid_y_L,FileNameBase,PathNameBase] = line_grid(FileNameBase, PathNameBase, im_grid, grid_x_L, grid_y_L);
        
    end
    % Remove Markers
    if gridselection==5
    [grid_x_L,grid_y_L,FileNameBase,PathNameBase] = removepoints(FileNameBase, PathNameBase, im_grid, grid_x_L, grid_y_L);
...
%% Select a rect area
function [grid_x_l,grid_y_l,FileNameBase,PathNameBase] = rect_grid(FileNameBase, PathNameBase, im_grid, grid_x_l, grid_y_l)
...
confirmselection = menu(sprintf('Do you want to use this grid?'),...
    ...
    if confirmselection==1
        ...
        close all
        hold off
        gridtypeselection(FileNameBase, PathNameBase, im_grid, grid_x_l, grid_y_l,0);
    end;
    ...
    if confirmselection==2
        ...
        plot(grid_x_l,grid_y_l,'+r');
        hold off;
        rect_grid(FileNameBase, PathNameBase, im_grid, grid_x_l, grid_y_l);
    end;
...
function [grid_x_l,grid_y_l,FileNameBase,PathNameBase] = circ_grid(FileNameBase, PathNameBase, im_grid, grid_x_l, grid_y_l)
...
confirmcircselection = menu(sprintf('Do you want to use this circle as basis?'),...
    ...
    if confirmcircselection==2
        ...
        plot(grid_x_l,grid_y_l,'+r');
        hold off
        circ_grid(FileNameBase, PathNameBase, im_grid, grid_x_l, grid_y_l);
    end
    ...
    if confirmcircselection==1
        ...
        if confirmselection==1
            ...
            grid_x_l=[grid_x_l;x];
            grid_y_l=[grid_y_l;y];
            save grid_x.dat grid_x_l -ascii -tabs
            save grid_y.dat grid_y_l -ascii -tabs
            close all
            hold off
            gridtypeselection(FileNameBase, PathNameBase, im_grid, grid_x_l, grid_y_l,0);
        end
...
%% Select 2 Points
function [grid_x_l,grid_y_l,FileNameBase,PathNameBase] = twop_grid(FileNameBase, PathNameBase, im_grid, grid_x_l, grid_y_l)
...
confirmselection = menu(sprintf('Do you want to use this grid?'),...
    ...
    if confirmselection==2
        ...
        plot(grid_x_l,grid_y_l,'+r');
        hold off;
        twop_grid(FileNameBase, PathNameBase, im_grid, grid_x_l, grid_y_l);
    end
...
%% Select a line
function [grid_x_l,grid_y_l,FileNameBase,PathNameBase] = line_grid(FileNameBase, PathNameBase, im_grid, grid_x_l, grid_y_l)
...
confirmselection = menu(sprintf('Do you want to use this grid?'),...
    ...
    if confirmselection==2
        ...
        plot(grid_x_l,grid_y_l,'+r');
        hold off;
        line_grid(FileNameBase, PathNameBase, im_grid, grid_x_l, grid_y_l);
    end
...
function [grid_x_l,grid_y_l,FileNameBase,PathNameBase] = removepoints(FileNameBase, PathNameBase, im_grid, grid_x_l, grid_y_l)
